...
function openModalHelper(args) {
  let ticketText = "Enter your text";
  let ticketColor = "";
  if (args) {
    // console.log(JSON.parse(args));
    args = JSON.parse(args);
    ticketText = args.ticketValue;
    ticketColor = args.ticketFilter;
  }
  ...
  ticketModal.innerHTML = `
        ...
        <div class="ticket-text" contenteditable="true" spellcheck="false" >${ticketText}
        </div>
  ...
  if (ticketColor != "") {
    ...
    ticketFilters[0].classList.remove("selected-filter");
    for (let index = 0; index < ticketFilters.length; index++) {
      if (ticketFilters[index].classList.contains(ticketColor)) {
        ticketFilters[index].classList.add("selected-filter");
        break;
      }
    }
  }
      ...
      if (e.keyCode == 13) {
        ...
        if (allTickets) {
          ...
          let newAllTickets = allTickets.map((obj) =>
            obj.ticketId == args.ticketId ? (obj = ticketObj) : obj
          );
...
function openTicketModal(editableContent) {
  // console.log(e);
  if (ticketModalOpen) {
    return;
  }
  if (editableContent.length > 0) {
    // console.log(JSON.parse(editableContent));
    openModalHelper(editableContent);
  } else {
    // console.log("Opened in non-editable mode");
    openModalHelper(null);
  }
}
...
function edit_ticket(ticketId) {
  // console.log(ticketId);
  let allTickets = myDB.getItem("allTickets");
  if (allTickets) {
    allTickets = JSON.parse(allTickets);
    // console.log(allTickets,typeof(allTickets))
    let ticketinfo;
    for (let i = 0; i < allTickets.length; i++) {
    ...
    openTicketModal(JSON.stringify(ticketinfo));
  }
}
         ...
         <div class="ticket-header ${ticketFilter}"></div>
            <div class="ticket-content">
                <div class="ticket-info">
                    ...
                    <div class="ticket-edit" onclick="edit_ticket('${ticketId}')"><i class="fa-solid fa-pen-to-square"></i></div>
